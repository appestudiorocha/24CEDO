		...
		$.ajax({
			...
			success: function (response) {
				...
				if(response.resultado == 0) {
					...
					$('#alert1').slideToggle();
					$('#alert2').slideToggle();
					//$('#nombreAlert2').html(response.data.apellido);
					console.log()
					if(leng == "en") {
						$('#Final').html("If it is necessary Dr. "+response.data.apellido+" will visit you at your home.");
					} else {
						$('#Final').html("De ser necesario el Dr. "+response.data.apellido+" lo visitará en su hogar.");
					}
					$('#imagenAlert2').attr("src",url + "/" + response.data.imagen);
				} else {
					switch (leng) {
						case "es":						
						//alert(response.es);
						navigator.notification.alert(response.es, null, "24 CEDO", "Cerrar");						
						break;
						case "en":						
						//alert(response.en);
						navigator.notification.alert(response.en, null, "24 CEDO", "Close");						
						break;
						case "pt":						
						//alert(response.pt);
						navigator.notification.alert(response.pt, null, "24 CEDO", "Fechar");						
						break;		
						case "ha":						
						//alert(response.ha);
						navigator.notification.alert(response.ha, null, "24 CEDO", "Close");						
						break;	
						default:
						navigator.notification.alert(response.en, null, "24 CEDO", "Close");
						break;
	...
	document.addEventListener('deviceready', function () {    		
		...
		if(usuario) {
			window.plugins.OneSignal.sendTag("cod", usuarioFinal[data].data.idUser);
		}
